...
def train_one_epoch(model, optimizer, data_loader, device, epoch, print_freq, lr_scheduler=None,
    ...
    # lr_scheduler = None removed by AAC
    if epoch == 0:
    ...
    for images, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        total_loss += loss_value  # Added AAC
    ...
    return total_loss / len(data_loader), metric_logger
...
def evaluate(model, data_loader, device):
    ...
    # Added by AAC
    stats_summ = coco_evaluator.summarize()

    torch.set_num_threads(n_threads)
    return coco_evaluator, stats_summ
